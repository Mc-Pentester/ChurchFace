...
export default function PrayerChainSettingsPage() {
  ...
  const fetchChain = async () => {
    try {
      const res = await fetch(`/api/prayers/chain/${chainId}`);
      const data = await res.json();
      setChain(data.chain);
      
      // Load currently associated rooms
      const roomsRes = await fetch(`/api/prayers/chain/${chainId}/rooms`);
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const res = await fetch(`/api/prayers/chain/${chainId}/rooms`, {
        method: "PUT",
      ...
      if (res.ok) {
        router.push(`/prayers/chains/${chainId}`);
      }
    } catch (error) {
    ...
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      ...
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900">
          Paramètres: {chain?.title}
        </h1>
        <p className="text-gray-600 mt-1">
          Associez des salles de prière à cette chaîne
        </p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200">
        <div className="p-4 border-b border-gray-200">
          <h2 className="font-semibold text-gray-900">Salles disponibles</h2>
          <p className="text-sm text-gray-600 mt-1">
            Sélectionnez les salles à associer à cette chaîne
          </p>
        </div>
