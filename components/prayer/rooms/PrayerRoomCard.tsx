...
import { Video, Mic, MessageSquare, Users, Lock, Globe, Share2, Check } from "lucide-react";
...
export function PrayerRoomCard({ room, onJoin, onView, onShare }: PrayerRoomCardProps) {
  const Icon = ROOM_TYPE_ICONS[room.roomType as keyof typeof ROOM_TYPE_ICONS] || MessageSquare;
  const typeLabel = ROOM_TYPE_LABELS[room.roomType as keyof typeof ROOM_TYPE_LABELS] || "Texte";
        ...
        <div className="px-4 py-3 bg-gray-50 space-y-2">
          <div className="flex items-center justify-between text-sm">
            <div className="flex items-center gap-2 text-gray-600">
              ...
              <span>{room._count?.participants || 0} participant{(room._count?.participants || 0) !== 1 ? "s" : ""}</span>
            </div>
          </div>
        ...
        <div className="p-4 flex gap-2 flex-col sm:flex-row">
          {isActive ? (
            ...
            <button
              ...
              className="flex-1 bg-gray-600 text-white py-2 px-4 rounded-lg font-medium hover:bg-gray-700 transition-colors w-full sm:w-auto"
            >
              Voir l'historique
            </button>
          )}
