"use client";

import { PrayerRoom } from "@/types/prayer";
import { Video, Mic, MessageSquare, Users, Lock, Globe, Clock } from "lucide-react";

interface PrayerRoomCardProps {
  room: PrayerRoom;
  onJoin?: (roomId: string) => void;
  onView?: (roomId: string) => void;
}

const ROOM_TYPE_ICONS = {
  TEXT: MessageSquare,
  AUDIO: Mic,
  VIDEO: Video,
};

const ROOM_TYPE_LABELS = {
  TEXT: "Texte",
  AUDIO: "Audio",
  VIDEO: "Vidéo",
};

export function PrayerRoomCard({ room, onJoin, onView }: PrayerRoomCardProps) {
  const Icon = ROOM_TYPE_ICONS[room.roomType];
  const typeLabel = ROOM_TYPE_LABELS[room.roomType];
  const isActive = room.isActive && (!room.endedAt || new Date(room.endedAt) > new Date());

  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
      {/* Header */}
      <div className="p-4 border-b border-gray-100">
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <div className="flex items-center gap-2">
              <h3 className="font-semibold text-gray-900 text-lg">{room.title}</h3>
              <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                isActive ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-600"
              }`}>
                {isActive ? "Actif" : "Terminé"}
              </span>
            </div>
            {room.description && (
              <p className="text-sm text-gray-600 mt-1 line-clamp-2">{room.description}</p>
            )}
          </div>
          <div className="flex items-center gap-1 text-gray-500 ml-3">
            <Icon className="w-5 h-5" />
            <span className="text-sm">{typeLabel}</span>
          </div>
        </div>
      </div>

      {/* Info */}
      <div className="px-4 py-3 bg-gray-50 space-y-2">
        <div className="flex items-center justify-between text-sm">
          <div className="flex items-center gap-2 text-gray-600">
            <Users className="w-4 h-4" />
            <span>{room._count?.participants || 0} participant{room._count?.participants !== 1 ? "s" : ""}</span>
          </div>
          {room.maxParticipants && (
            <span className="text-gray-500">
              {room._count?.participants || 0}/{room.maxParticipants}
            </span>
          )}
        </div>

        {room.scheduledStart && (
          <div className="flex items-center gap-2 text-sm text-gray-600">
            <Clock className="w-4 h-4" />
            <span>
              {new Date(room.scheduledStart).toLocaleString()} -{" "}
              {room.scheduledEnd ? new Date(room.scheduledEnd).toLocaleString() : ""}
            </span>
          </div>
        )}

        <div className="flex items-center gap-2 text-sm text-gray-500">
          {room.isPublic ? (
            <>
              <Globe className="w-4 h-4" />
              <span>Public</span>
            </>
          ) : (
            <>
              <Lock className="w-4 h-4" />
              <span>Privé</span>
            </>
          )}
        </div>
      </div>

      {/* Actions */}
      <div className="p-4 flex gap-2">
        {isActive ? (
          <button
            onClick={() => onJoin?.(room.id)}
            className="flex-1 bg-blue-600 text-white py-2 px-4 rounded-lg font-medium hover:bg-blue-700 transition-colors"
          >
            Rejoindre
          </button>
        ) : (
          <button
            onClick={() => onView?.(room.id)}
            className="flex-1 bg-gray-600 text-white py-2 px-4 rounded-lg font-medium hover:bg-gray-700 transition-colors"
          >
            Voir l'historique
          </button>
        )}
      </div>
    </div>
  );
}
